...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(LOGIN_UPD_MUTE_TIME_PREMIUM, "UPDATE account_premium SET time = ? WHERE id = ?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(LOGIN_SEL_PINFO, "SELECT a.username, aa.gmlevel, a.email, a.last_ip, DATE_FORMAT(a.last_login, '%Y-%m-%d %T'), a.mutetime, a.dp, a.vp FROM account a LEFT JOIN account_access aa ON (a.id = aa.id AND (aa.RealmID = ? OR aa.RealmID = -1)) WHERE a.id = ?", CONNECTION_SYNCH);
