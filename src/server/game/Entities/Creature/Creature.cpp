...
bool Creature::InitEntry(uint32 Entry, uint32 /*team*/, const CreatureData* data)
{
    ...
    if(data)
        SetFloatValue(OBJECT_FIELD_SCALE_X, data->scale);
    else
            ...
            if (GetCreatureTemplate()->trainer_race && player->getRace() != GetCreatureTemplate()->trainer_race)
            {
                if (msg)
                {
                    ...
                    switch (GetCreatureTemplate()->trainer_class)
                    {
                        ...
                        case RACE_NAGA:         player->PlayerTalkClass->SendGossipMenu(5862, GetGUID()); break;
                        case RACE_VRYKUL:       player->PlayerTalkClass->SendGossipMenu(5864, GetGUID()); break;
                        case RACE_GOBLIN:       player->PlayerTalkClass->SendGossipMenu(5816, GetGUID()); break;
                        case RACE_BROKEN:       player->PlayerTalkClass->SendGossipMenu(5861, GetGUID()); break;
                    }
                }
                return false;
            }
...
void Creature::SaveToDB(uint32 mapid, uint8 spawnMask, uint32 phaseMask)
{
    ...
    data.scale = GetFloatValue(OBJECT_FIELD_SCALE_X);
