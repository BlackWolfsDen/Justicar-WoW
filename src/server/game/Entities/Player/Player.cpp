...
#include "Config.h"
//#include "../scripts/Custom/dire_maul.h"
...
#define MAX_DEATH_COUNT 1

static uint32 copseReclaimDelay[MAX_DEATH_COUNT] = { 22 };
...
void Player::Update(uint32 p_time)
{
    ...
    if ((getRace() == RACE_BROKEN && getGender() == GENDER_FEMALE) || (getRace() == RACE_GOBLIN && getGender() == GENDER_FEMALE))
    {
	 SetByteValue(UNIT_FIELD_BYTES_0, 2, GENDER_MALE);
        SetByteValue(PLAYER_BYTES_3, 0, GENDER_MALE);
        // Change display ID
        InitDisplayIds();
    ...
    if (getClass() == CLASS_MAGE && !HasAura(93002))
    {
	 CastSpell(this,93002,true);
    }

    if(HasAura(37284) && !IsInWater())
	RemoveAura(37284);
...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options)
{
    ...
    /*if(isBattleActive)
    {
	ChatHandler(this).PSendSysMessage("Someone is currently running the Dire Maul Battle. You will have to wait till he has finished.");
        return false;
    }*/
    ...
    {
	ChatHandler(this).PSendSysMessage("You are not allowed to fly into this area.");
	Dismount();
    ...
    if((!player->HasAura(8221) && !player->HasAura(8222) && !player->HasAura(8219) && !player->HasAura(8220)))
    {
       DeMorph();
    }
...
void Player::SetDevFlag(bool on)
{
    if (on)
    {
        m_ExtraFlags |= PLAYER_EXTRA_GM_ON;
...
void Player::GiveXP(uint32 xp, Unit* victim, float group_rate)
{
    ...
    if (HasAura(24705))
        favored_exp_mult = 0.15f; // invocation of the wickerman
    if (zone == 2677)
	 favored_exp_mult = 0.10f;
...
void Player::ClearBattlegroundChars()
{
    // Run these queries as an attempt to repair issue #19
    CharacterDatabase.PExecute("UPDATE characters SET map = '534', position_x = '5541,770020', position_y = '-3783,469971', position_z = '1594,910034', orientation = '2,883010' WHERE map = '489' AND online = '0'");
    CharacterDatabase.PExecute("UPDATE characters SET map = '534', position_x = '5541,770020', position_y = '-3783,469971', position_z = '1594,910034', orientation = '2,883010' WHERE map = '529' AND online = '0'");
    CharacterDatabase.PExecute("UPDATE characters SET map = '534', position_x = '5541,770020', position_y = '-3783,469971', position_z = '1594,910034', orientation = '2,883010' WHERE map =  '30' AND online = '0'");
    CharacterDatabase.PExecute("UPDATE characters SET map = '534', position_x = '5541,770020', position_y = '-3783,469971', position_z = '1594,910034', orientation = '2,883010' WHERE map = '607' AND online = '0'");
    CharacterDatabase.PExecute("UPDATE characters SET map = '534', position_x = '5541,770020', position_y = '-3783,469971', position_z = '1594,910034', orientation = '2,883010' WHERE map = '566' AND online = '0'");
    return;
}
...
void Player::ResurrectPlayer(float restore_percent, bool applySickness)
{
    ...
    if (GetMapId() == 13)
    {
	 CastSpell(this, 42354, true);
	 TeleportTo(13, -333.532349f, -49.794476f, 626.979309f, 1.972229f);
	 ChatHandler(this).PSendSysMessage("You have died, and the elemntal gods has bound your soul for 45 seconds.");
    }
...
void Player::GetDodgeFromAgility(float &diminishing, float &nondiminishing)
{
    ...
    diminishing = 40.0f * bonus_agility * dodgeRatio->ratio * crit_to_dodge[pclass-1];
    nondiminishing = 40.0f * (dodge_base[pclass-1] + base_agility * dodgeRatio->ratio * crit_to_dodge[pclass-1]);
}
...
bool Player::RewardHonor(Unit* uVictim, uint32 groupsize, int32 honor, bool pvptoken)
{
	...
	if(uVictim)
	{
		if(uVictim->GetTypeId() == TYPEID_PLAYER)
		{
			...
			{
				ChatHandler(this).PSendSysMessage("You can not get honor from killing someone using your own IP-Address.");
				return false;
			}
		}
	}
    ...
    if (honor_f <= 0)
    {
        ...
        if (uVictim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (GetTeam() == victim->GetTeam() && GetAreaId() != isPVPArea(GetAreaId()) && victim->GetAreaId() != isPVPArea(victim->GetAreaId()))
                return false;
...
bool Player::isPVPArea(uint32 newArea)
{
	QueryResult result;
	result = WorldDatabase.PQuery("SELECT * FROM area_pvp where areaId = %u", newArea);
	if(result)
		return true;
...
bool Player::isSanctuaryArea(uint32 newArea)
{
	QueryResult result;
	result = WorldDatabase.PQuery("SELECT * FROM area_sanctuary where areaId = %u", newArea);
	if(result)
		return true;
...
void Player::UpdateArea(uint32 newArea)
{
    ...
    if(NotInPvPMall(newArea))m_areaUpdateId = newArea;
    ...
    //if (area && area->IsSanctuary() || newArea == 4161 || newArea == 4575 || newArea == 4430 || newArea == 4862 || newArea == 2317 || newArea == 255 || newArea == 3606 || newArea == 3708 || newArea == 3709 || newArea == 3710)    // in sanctuary
    if (area && area->IsSanctuary() || isSanctuaryArea(newArea) || GetMapId() == 573)
    {
...
bool Player::NotInPvPMall(uint32 & newArea)
{
	//if(newArea == 150 || newArea == 298 || newArea == 876 || newArea == 2266 || newArea == 2408 || newArea == 598 || newArea == 4217)
	if(isPVPArea(newArea))
	{
		newArea = 2177;
		return false;
	}
...
InventoryResult Player::CanEquipItem(uint8 slot, uint16 &dest, Item* pItem, bool swap, bool not_loading) const
{
    ...
    if (pItem)
    {
        ...
        if (pProto)
        {
            ...
            // custom
            QueryResult result = WorldDatabase.PQuery("SELECT security FROM item_vip WHERE entry = %u", pItem->GetEntry());
            if(result && result->Fetch()[0].GetUInt8() > (uint8)GetSession()->GetSecurity())
                return EQUIP_ERR_CANT_EQUIP_NEED_VIP;
...
void Player::SetVisibleItemSlot(uint8 slot, Item* pItem)
{
    if (pItem)
    {
	 // Transmo
        ...
        {
            CharacterDatabase.PExecute("UPDATE fake_items SET fakeEntry = 0 WHERE guid = %u", pItem->GetGUIDLow());
...
{
    if(msg == EQUIP_ERR_CANT_EQUIP_NEED_VIP) // custom
    {
        GetSession()->SendNotification("You need to be VIP to equip that");
        msg = EQUIP_ERR_NONE; // send no equip error
    }
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (Item* item = _LoadItem(trans, zoneId, timeDiff, fields))
            {
                ...
                if(QueryResult queryResult = CharacterDatabase.PQuery("SELECT `fakeEntry` FROM `fake_items` WHERE `guid` = '%u'",item->GetGUIDLow()))
		            item->FakeEntry = queryResult->Fetch()[0].GetUInt32();
                else
                    item->FakeEntry = NULL;
...
void Player::ClearAllCooldowns()
{
    for (SpellCooldowns::const_iterator itr=m_spellCooldowns.begin(); itr != m_spellCooldowns.end(); ++itr)
    {
		...
		this->SendClearCooldown(sEntry->Id, this);
		//this->RemoveSpellCooldown(sEntry->Id, true);
	}
	m_spellCooldowns.clear();
}
...
void Player::ModifyMoney(int32 d)
{
    ...
    else
    {
        ...
        else
        {
	         ...
	         if(d)
	         newAmount = 0; // new amount of gold +14k gold messed up
	         GoldLimitString = "You have reached maximum amount of gold, instead you got 4 gold bars!";
	         m_session->SendAreaTriggerMessage(GoldLimitString.c_str());
		  ChatHandler(player->GetSession()).PSendSysMessage("You have reached maximum amount of gold, instead you got 4 gold bars!");
	         m_session->GetPlayer()->AddItem(55004, 4);  // item ID x4 (200k gold)
        }
        SetMoney (newAmount);
    }
}
...
void Player::UpdateAreaDependentAuras(uint32 newArea)
{
	...
	if (HasAuraType(SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED))
	{
		QueryResult PM_QUERY = WorldDatabase.PQuery("SELECT * FROM _pm_no_fly where area_id = %u", newArea);
		if(PM_QUERY)
		{
			CastSpell(this, 12438, true);
			ChatHandler(this).PSendSysMessage("You are not allowed to fly into this area.");
			Dismount();
			RemoveAurasByType(SPELL_AURA_MOUNTED);
		}
...
{
    if(!map){
        sLog->outCrash("No map was found for the player %s with guid %u.",GetName(),GetGUIDLow());
        return;
    }
    Unit::SetMap(map);
...
bool Player::GetCommandStatus(int command)
{
    ...
    case CHEAT_GOD: return m_cheatGod;
        break;
    case HIDE_WORLD: return m_hideWorld;
        break;
    case HIDE_VIP: return m_hideVip;
        break;
...
uint32 Player::GetDonationPoints(uint32 PlayerId)
{
	uint32 dp;
...
uint32 Player::GetVotingPoints(uint32 PlayerId)
{
	uint32 vp;
