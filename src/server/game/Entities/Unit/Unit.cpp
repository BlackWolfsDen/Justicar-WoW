...
uint32 Unit::SpellCriticalDamageBonus(SpellInfo const* spellProto, uint32 damage, Unit* victim)
{
    ...
    switch (spellProto->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_RANGED:
            ...
            break;
        default:
            crit_bonus += damage * 15;                       // for spells is 50%
            break;
    }
...
void Unit::SetLevel(uint8 lvl)
{
    ...
    if(GetTypeId() == TYPEID_PLAYER && lvl == 10)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have leveled so fast your character has gone berserk!", ToPlayer());
	plr->AddAura(24378, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 15)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You got wings!", ToPlayer());
	plr->AddAura(43430, plr);
	plr->AddAura(26393, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 20)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have leveled so fast your character has gone berserk!", ToPlayer());
	plr->AddAura(24378, plr);
	plr->PlayDirectSound(11962, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 30)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have reached the level of speed!", ToPlayer());
	plr->AddAura(62471, plr);
	plr->AddAura(26035, plr);
	plr->AddAura(69052, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 35)
    {
	sWorld->SendServerMessage(SERVER_MSG_STRING, "[|cffff0000Nexus Whispers|cffFFFF05]|cffff0000:|cffFFFF05 Very good my dear player, you have been able to level to 35. Now if you must know, one of my friends |cffff0000Trosek|cffFFFF05 lives in these areas. He has some troubles with some living trees, i guess if you find him and slay the trees he will reward you with some awesome weapons! Have fun!", ToPlayer());
	plr->AddAura(31305, plr);
	plr->AddAura(43430, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 40)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have leveled so fast your character has gone berserk!", ToPlayer());
	plr->AddAura(24378, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 45)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "The kings are watching you!", ToPlayer());
	plr->AddAura(43223, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 50)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have leveled so fast your character has gone berserk!", ToPlayer());
	plr->AddAura(24378, plr);
	plr->AddAura(62471, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 55)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have started to warp!", ToPlayer());
	plr->AddAura(36479, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 60)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have leveled so fast your character has gone berserk!", ToPlayer());
	plr->AddAura(24378, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 70)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have reached the level of speed!", ToPlayer());
	plr->AddAura(62471, plr);
	plr->AddAura(26035, plr);
    ...
    if(GetTypeId() == TYPEID_PLAYER && lvl == 75)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have been blessed by elune!", ToPlayer());
	plr->AddAura(26393, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 80)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "Dear warrior you have reached level 80, the gods has teleported you to the Path of the Damned. Here you fill find what your next task is.. !", ToPlayer());
	plr->TeleportTo(530, 6654.645508f, -6458.209473f, 29.259302f, 2.664963f);
	plr->PlayDirectSound(11472, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 85)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have started to warp!", ToPlayer());
	plr->AddAura(36479, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 90)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You got wings!", ToPlayer());
	plr->AddAura(43430, plr);
	plr->AddAura(31884, plr);
	plr->PlayDirectSound(11802, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 95)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "The kings are watching you!", ToPlayer());
	plr->AddAura(43223, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 100)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have reached the level of speed!", ToPlayer());
	plr->AddAura(62471, plr);
	plr->AddAura(43430, plr);
    ...
    if(GetTypeId() == TYPEID_PLAYER && lvl == 105)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have started to warp!", ToPlayer());
	plr->AddAura(36479, plr);
	plr->AddAura(43223, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 120)
    {
	sWorld->SendServerMessage(SERVER_MSG_STRING, "[|cffff0000Nexus Whispers|cffFFFF05]|cffff0000:|cffFFFF05 Very good my dear player, you have been able to level to 120. You are probably hungering for a real battle. Maybe a dragon will be good? I heard |cffff0000Sartharion|cffFFFF05 is on the run, find |cffff0000Goliath|cffFFFF05 he will guide you! Good luck!", ToPlayer());
	plr->AddAura(24378, plr);
	plr->AddAura(26035, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 140)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have leveled so fast your character has gone berserk!", ToPlayer());
	plr->AddAura(24378, plr);
	plr->AddAura(26035, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 150)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You got wings!", ToPlayer());
	plr->AddAura(43430, plr);
	plr->AddAura(31884, plr);
    ...
    if(GetTypeId() == TYPEID_PLAYER && lvl == 160)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have leveled so fast your character has gone berserk!", ToPlayer());
	plr->AddAura(24378, plr);
	plr->AddAura(62471, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 165)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "The kings are watching you!", ToPlayer());
	plr->AddAura(43223, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 170)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have leveled so fast your character has gone berserk!", ToPlayer());
	plr->AddAura(24378, plr);
	plr->AddAura(26035, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 180)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have leveled so fast your character has gone berserk!", ToPlayer());
	plr->AddAura(24378, plr);
	plr->AddAura(26035, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 185)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You got wings!", ToPlayer());
	plr->AddAura(43430, plr);
	plr->AddAura(31884, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 190)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have started to warp!", ToPlayer());
	plr->AddAura(36479, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 200)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have reached level 200, once again the gods calls for you. This time you must defeat the monster Onyxia!", ToPlayer());
	plr->TeleportTo(249, 29.160700f, -71.337196f, -8.180320f, 4.435840f);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 210)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have started to warp!", ToPlayer());
	plr->AddAura(36479, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 220)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have leveled so fast your character has gone berserk!", ToPlayer());
	plr->AddAura(24378, plr);
	plr->AddAura(26035, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 225)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "The kings are watching you!", ToPlayer());
	plr->AddAura(43223, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 240)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You have leveled so fast your character has gone berserk!", ToPlayer());
	plr->AddAura(24378, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 245)
    {
       sWorld->SendServerMessage(SERVER_MSG_STRING, "You got wings!", ToPlayer());
	plr->AddAura(43430, plr);
	plr->AddAura(31884, plr);
    }
    if(GetTypeId() == TYPEID_PLAYER && lvl == 255)
    {
	sWorld->SendServerMessage(SERVER_MSG_STRING, "[|cffff0000Nexus Whispers|cffFFFF05]|cffff0000:|cffFFFF05 Very good my dear player, you have been able to level to 255, that is the maximum level. You can go find the |cffff0000255 Starter Vendor|cffFFFF05 he sells you your very first level 255 armor set, for free! After you have geared up, your own your way to the |cffff0000Vault of Archavon|cffFFFF05! Just goto the teleporter and press |cffff0000High Level Instances|cffFFFF05 -> |cffff0000Vault of Archavon|cffFFFF05!", ToPlayer());
	plr->TeleportTo(1, -3673.392090f, -4384.723145f, 10.02678f, 3.879712f);
	plr->PlayDirectSound(11466, plr);
    }
