...
void ObjectMgr::LoadCreatureTemplates()
{
    ...
    do
    {
        ...
        creatureTemplate.minlevel          = fields[14].GetUInt16();
        creatureTemplate.maxlevel          = fields[15].GetUInt16();
...
void ObjectMgr::LoadCreatures()
{
    ...
    do
    {
	 ...
	 data.scale          = fields[22].GetFloat();
        ...
        if (data.scale > 20.0f || data.scale < 0.1f)
        {
            sLog->outErrorDb("Table `creature` have creature (GUID: %u Entry: %u) with `size` > 20.0f or `size` > 0.1 (not in valid range), set to 1.", guid, data.id);
            data.scale = 1;
        }
...
void ObjectMgr::LoadGameobjects()
{
    ...
    do
    {
        ...
        if (data.size < 0.0f)
        {
            //sLog->outError(LOG_FILTER_SQL, "Table `gameobject` has gameobject (GUID: %u Entry: %u) with invalid size (%f) value, skip", guid, data.id, data.size);
            continue;
        }
...
void ObjectMgr::LoadPlayerInfo()
{
    ...
    sLog->outString("Loading Player Create Level Stats Data...");
    {
        ...
        do
        {
            ...
            uint32 current_level = fields[2].GetUInt16();
            if (current_level > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                if (current_level > STRONG_MAX_LEVEL)        // hardcoded level maximum
