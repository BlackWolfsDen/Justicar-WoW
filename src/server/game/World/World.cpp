...
void World::LoadConfigSettings(bool reload)
{
    ...
    std::string s_pvp_ranks = ConfigMgr::GetStringDefault("PvPRank.HKPerRank", "10,50,150,200,300,400,500,600,700,800,900,1000,1250,1500");
    ...
    m_int_configs[CONFIG_OUTDOOR_PVP_BT_HONOR] = ConfigMgr::GetIntDefault("OutDoorPVPBT.Honor", 500);
    m_int_configs[CONFIG_OUTDOOR_PVP_BT_ARENA] = ConfigMgr::GetIntDefault("OutDoorPVPBT.Arena", 250);
...
void World::SendGlobalGMMessage(WorldPacket* packet, WorldSession* self, uint32 team)
{
    ...
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        if (itr->second &&
            ...
            itr->second->GetPlayer()->IsInWorld() &&
            itr->second != self &&
            itr->second->GetSecurity() >= SEC_GAMEMASTER &&
            (team == 0 || itr->second->GetPlayer()->GetTeam() == team))
        {
            itr->second->SendPacket(packet);
        }
...
void World::SendGlobalVIPMessage(WorldPacket* packet, WorldSession* self, uint32 team)
{
    SessionMap::iterator itr;
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
...
void World::SendWorldText(int32 string_id, ...)
{
    ...
    for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
	 ...
	 if (itr->second->GetPlayer()->GetCommandStatus(HIDE_WORLD) == false)
	     continue;
