...
void Spell::SelectImplicitChannelTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    switch (targetType.GetTarget())
    {
        case TARGET_UNIT_CHANNEL_TARGET:
            ...
            if (Unit* target = Unit::GetUnit(*m_caster, m_originalCaster->GetUInt64Value(UNIT_FIELD_CHANNEL_OBJECT)))
...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
	...
	if (m_caster->GetTypeId() == TYPEID_PLAYER)
	{
		...
		{
			if((player->IsPvP() || player->IsOutdoorPvPActive() || player->GetMap()->IsBattlegroundOrArena() || player->pvpInfo.inFFAPvPArea || player->pvpInfo.inHostileArea || player->GetAreaId() == 255  || player->GetMapId() == 573 || player->GetMapId() == 631) && player->GetAreaId() != 3217)
			{
				player->m_CheatCastTime = false;
				player->m_CheatCooldown = false;
				player->m_cheatPower = false;
				ChatHandler(player).PSendSysMessage("Your cheats has been cancelled, your not allowed to have them turned on when your in PvP mode.");
			}
			else
			{
				m_casttime = 0;
				if(m_caster->ToPlayer()->GetCommandStatus(CHEAT_COOLDOWN))
				   player->SendClearCooldown(GetSpellInfo()->Id, player);
			}
		}
	}
...
void Spell::finish(bool ok)
{
	...
	{
		Player* player = m_caster->ToPlayer();
		if(player && player->GetCommandStatus(CHEAT_COOLDOWN))
		{
			if((player->IsPvP() || player->IsOutdoorPvPActive() || player->GetMap()->IsBattlegroundOrArena() || player->pvpInfo.inFFAPvPArea || player->pvpInfo.inHostileArea || player->GetAreaId() == 255 || player->GetMapId() == 573 || player->GetMapId() == 631) && player->GetAreaId() != 3217)
			{
				player->m_CheatCooldown = false;
...
void Spell::TakeCastItem()
{
    ...
    if(m_caster->ToPlayer()->GetCommandStatus(CHEAT_COOLDOWN) || m_caster->ToPlayer()->GetCommandStatus(CHEAT_CASTTIME))
    {
	 if(m_caster->ToPlayer()->GetCommandStatus(CHEAT_COOLDOWN))
           m_caster->ToPlayer()->m_CheatCooldown = false;
	 if(m_caster->ToPlayer()->GetCommandStatus(CHEAT_CASTTIME))
           m_caster->ToPlayer()->m_CheatCastTime = false;
        sLog->outError("Player %s abused an item Name: %s while having cheats on.", m_caster->ToPlayer()->GetName(), proto->Name1.c_str());
	 ChatHandler(m_caster->ToPlayer()).PSendSysMessage("Your cheats has been cancelled, your not allowed to use items when cheats are on. Your actions have been logged, continue this action and your cheats will be taken from you.");
    }
...
SpellCastResult Spell::CheckCast(bool strict)
{
	...
	uint32 PM_MOUNT_SPELL [13]	  = {0, 40192,63844,61309,72807,74856,59567,93025,93026,93027,93054,93061,93065};
	for(uint32 PM_Count = 1; PM_Count < 13; PM_Count++)
	{
		if(m_spellInfo->Id == PM_MOUNT_SPELL[PM_Count])
		{
			locRes = SPELL_CAST_OK;

			QueryResult PM_QUERY = WorldDatabase.PQuery("SELECT * FROM _pm_no_fly where area_id = %u", area);
			if(PM_QUERY)
				return SPELL_FAILED_INCORRECT_AREA;

			if(m_caster->GetMapId() == 573)
				return SPELL_FAILED_INCORRECT_AREA;
		}
	}
    }
