...
bool Map::AddPlayerToMap(Player* player)
{
    ...
    {
	 player->RemoveAura(61254);
	 player->TeleportTo(530, 6654.645508f, -6458.209473f, 29.259302f, 2.664963f);
    }
    if (player->GetMapId() == 532 || player->GetMapId() == 631 && (player->GetCommandStatus(CHEAT_COOLDOWN) || player->GetCommandStatus(CHEAT_CASTTIME)))
    {
...
void Map::RemovePlayerFromMap(Player* player, bool remove)
{
    ...
    if(player->HasAura(68378))
    {
	 player->RemoveAura(68378);
    }

    if(player->HasAura(5118))
    {
	 player->RemoveAura(5118);
    }
