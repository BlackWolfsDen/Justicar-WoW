...
void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket & recv_data)
{
    ...
    {
       item = GetPlayer()->GetItemByGuid(guid);
       if (!item)
	{
    ...
    if ((unit && unit->GetCreatureTemplate()->ScriptID != unit->LastUsedScriptID) || (go && go->GetGOInfo()->ScriptId != go->LastUsedScriptID))
    {
 	 ...
 	 if (item)
	     item->LastUsedScriptID = item->GetScriptId();
...
void WorldSession::HandleWhoOpcode(WorldPacket & recv_data)
{
    ...
    for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
    {
     ...
     if (itr->second->GetSession()->GetSecurity() == SEC_MODERATOR)
        ...
        if (itr->second->GetSession()->GetSecurity() == 3)
        {
            pname = "<Jr. GM>";
            pname.append(itr->second->GetName());
        }

        if (itr->second->GetSession()->GetSecurity() == 4)
        {
            pname = "<GM>";
            pname.append(itr->second->GetName());
        }

        if (itr->second->GetSession()->GetSecurity() == 5)
        {
            pname = "<Sr. GM>";
            pname.append(itr->second->GetName());
        }

        if (itr->second->GetSession()->GetSecurity() == 6)
        {
            pname = "<Admin>";
            pname.append(itr->second->GetName());
        }

        if (itr->second->GetSession()->GetSecurity() == 7)
        {
            pname = "<Sr. Admin>";
            pname.append(itr->second->GetName());
        }

        if (itr->second->GetSession()->GetSecurity() == 8)
        {
            pname = "<Developer>";
            pname.append(itr->second->GetName());
        }

        if (itr->second->GetSession()->GetSecurity() == 9)
        {
            pname = "<Head Admin>";
            pname.append(itr->second->GetName());
        }

        if (itr->second->GetSession()->GetSecurity() == 10)
        {
            pname = "<Owner>";
            pname.append(itr->second->GetName());
        }
...
void WorldSession::HandleTogglePvP(WorldPacket & recv_data)
{
    ...
    if(GetPlayer()->GetCommandStatus(CHEAT_COOLDOWN) && GetPlayer()->GetAreaId() != 3217)
    {
	    GetPlayer()->m_CheatCooldown = false;
	    ChatHandler(GetPlayer()).PSendSysMessage("You have been flagged for PVP, so your cooldown cheat has been turned off.");
    }

    if(GetPlayer()->GetCommandStatus(CHEAT_CASTTIME) && GetPlayer()->GetAreaId() != 3217)
    {
	    GetPlayer()->m_CheatCastTime = false;
	    ChatHandler(GetPlayer()).PSendSysMessage("You have been flagged for PVP, so your casttime cheat has been turned off.");
    }
