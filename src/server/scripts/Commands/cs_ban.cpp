...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        static ChatCommand banCommandTable[] =
        {
            ...
            { "vip",            SEC_ADMINISTRATOR,  true,  &HandleBanVIPCommand,                "", NULL },
            { NULL,             0,                  false, NULL,                                "", NULL }
        };
    ...
    static bool HandleBanVIPCommand(ChatHandler* handler, char const* args)
    {
        char* nameStr;
        char* delayStr;
        ...
        {
        handler->PSendSysMessage("You can't ban players from the vip channel.");
        ...
        PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_MUTE_TIME_PREMIUM);
        ...
        switch (sWorld->BanCharacter(name, durationStr, reasonStr, handler->GetSession() ? handler->GetSession()->GetPlayerName() : ""))
        {
            case BAN_SUCCESS:
            {
                if (atoi(durationStr) > 0)
                    sWorld->SendWorldText(LANG_BAN_YOUBANNED, name.c_str(), secsToTimeString(TimeStringToSecs(durationStr), true).c_str(), reasonStr);
                else
                    sWorld->SendWorldText(LANG_BAN_YOUPERMBANNED, name.c_str(), reasonStr);
                break;
            }
            case BAN_NOTFOUND:
            {
    ...
    static bool HandleBanHelper(BanMode mode, char const* args, ChatHandler* handler)
    {
        ...
        switch (sWorld->BanAccount(mode, nameOrIP, durationStr, reasonStr, handler->GetSession() ? handler->GetSession()->GetPlayerName() : ""))
        {
            case BAN_SUCCESS:
                if (atoi(durationStr) > 0)
                    sWorld->SendWorldText(LANG_BAN_YOUBANNED, nameOrIP.c_str(), secsToTimeString(TimeStringToSecs(durationStr), true).c_str(), reasonStr);
                else
                    sWorld->SendWorldText(LANG_BAN_YOUPERMBANNED, nameOrIP.c_str(), reasonStr);
                break;
