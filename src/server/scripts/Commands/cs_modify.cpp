...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        static ChatCommand modifyCommandTable[] =
        {
			...
			{ "vp",             SEC_ADMINISTRATOR,   false, &HandleModifyVPCommand,            "", NULL },
			{ "dp",             SEC_ADMINISTRATOR,   false, &HandleModifyDPCommand,     	   "", NULL },
	...
	/*
	* Modify the vote points by the player name.
	*/
    static bool HandleModifyVPCommand(ChatHandler* handler, const char* args)
    {
		if (!*args)
            return false;
		...
		Player* target = NULL;
        std::string plrName;
        pName = strtok((char*)args, " ");
        if (pName)
        {
            plrName = pName;
            normalizePlayerName(plrName);
            target = sObjectAccessor->FindPlayerByName(plrName.c_str()); //get player by name
        }else
        ...
        if (!target)
        {
            ...
            targetGuid = target->GetGUIDLow(); // assign guid

		if (handler->GetSession()->GetSecurity() < 7) // only 7,8,9,10 security levels
            return false;

		char* Value = strtok(NULL, " ");
        	int32 addVp = atoi(Value);
		if(!addVp){
			handler->PSendSysMessage("Please write a value for the vote points.");  
			return false;
		}
		if(!Value){
			handler->PSendSysMessage("Please write a value for the vote points.");  
		...
		// if the player already has vote points
		if(QueryResult result = CharacterDatabase.PQuery("SELECT vp FROM fusion.account_data WHERE id = '%u'", target->GetSession()->GetAccountId()))
		{
			Field *Fields = result->Fetch();
			uint32 playerVp = Fields[0].GetUInt32();
			if (addVp < 0) // remove vote points
			{
				if(playerVp == 0){ // player has nothing to remove.
					handler->PSendSysMessage("The player has zero vote points.");  
					return false;
				}
				int32 newVp = int32(playerVp) + addVp;
				if (newVp <= 0) // if all vote points are removed.
				{
					handler->PSendSysMessage("You removed all %s's vote points.", handler->GetNameLink(target).c_str());  
					CharacterDatabase.PExecute("UPDATE fusion.account_data SET vp = '0' WHERE id = '%u'", target->GetSession()->GetAccountId());
				}
				else // if some points removed
				{
					handler->PSendSysMessage("You removed %u from %s's vote points.", abs(addVp), handler->GetNameLink(target).c_str());
					CharacterDatabase.PExecute("UPDATE fusion.account_data SET vp = '%u' WHERE id = '%u'", newVp, target->GetSession()->GetAccountId());
				}
			}
			else
			{
				playerVp += addVp;
				handler->PSendSysMessage("You gave %s's %u vote points.", handler->GetNameLink(target).c_str(), addVp);  
				CharacterDatabase.PExecute("UPDATE fusion.account_data SET vp = '%u' WHERE id = '%u'", playerVp, target->GetSession()->GetAccountId());
			}
		}
        return true;
	}
	...
	/*
	* Modify the donation points by the player name.
	*/
    static bool HandleModifyDPCommand(ChatHandler* handler, const char* args)
    {
	 if (!*args)
            return false;
		...
		char* Value = strtok(NULL, " ");
        int32 addDp = atoi(Value);
		if(!addDp){
			handler->PSendSysMessage("Please write a value for the donation points.");  
			return false;
		}
		if(!Value){
			handler->PSendSysMessage("Please write a value for the vote points.");  
		...
		// if the player already has donation points
		if(QueryResult result = CharacterDatabase.PQuery("SELECT dp FROM fusion.account_data WHERE id = '%u'", target->GetSession()->GetAccountId()))
		{
			Field *Fields = result->Fetch();
			uint32 playerDp = Fields[0].GetUInt32();
			if (addDp < 0) // remove donation points
			{
				if(playerDp == 0){ // player has nothing to remove.
					handler->PSendSysMessage("The player has zero donation points.");  
					return false;
				}
				int32 newDp = int32(playerDp) + addDp;
				if (newDp <= 0) // if all donation points are removed.
				{
					handler->PSendSysMessage("You removed all %s's donation points.", handler->GetNameLink(target).c_str());  
					CharacterDatabase.PExecute("UPDATE fusion.account_data SET dp = '0' WHERE id = '%u'", target->GetSession()->GetAccountId());
				}
				else // if some donation points removed
				{
					handler->PSendSysMessage("You removed %u from %s's donation points.", abs(addDp), handler->GetNameLink(target).c_str());
					CharacterDatabase.PExecute("UPDATE fusion.account_data SET dp = '%u' WHERE id = '%u'", newDp, target->GetSession()->GetAccountId());
				}
			}
			else
			{
				playerDp += addDp;
				handler->PSendSysMessage("You gave %s's %u donation points.", handler->GetNameLink(target).c_str(), addDp);  
				CharacterDatabase.PExecute("UPDATE fusion.account_data SET dp = '%u' WHERE id = '%u'", playerDp, target->GetSession()->GetAccountId());
			}
		}
        return true;
	}
