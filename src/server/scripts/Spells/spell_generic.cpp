...
class spell_gen_friendlystep : public SpellScriptLoader
{
    public:
        spell_gen_friendlystep() : SpellScriptLoader("spell_gen_friendlystep") {}

        class spell_gen_friendlystep_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_gen_friendlystep_SpellScript);
		  ...
		  Player* target = ObjectAccessor::FindPlayer(player->GetSelection());
                if (!target || !player->IsFriendlyTo(target) || !target->IsInWorld() || target->isGameMaster() || target == player || target->GetGUID() == player->GetGUID()) // player checks
                {
                    player->ToPlayer()->GetSession()->SendNotification("You can only use friendlystep on a friendly player.");
                    return;
                }

                if (!player->IsWithinLOSInMap(target)) // If the target is not in los
                {
                    player->ToPlayer()->GetSession()->SendNotification("You can't see your target!");
                    return;
                }

                if (player->GetDistance(target) > 30.0f) // If the target is too far away
                {
                    player->ToPlayer()->GetSession()->SendNotification("Out of range max 30 yards.");
                    return;
                }

		  uint32 curValue = player->GetHealth();
                player->TeleportTo(target->GetMapId(), target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), player->GetAngle(target));
                player->CastSpell(player, 31643, true);
		  player->SetHealth(curValue*1.1);
            }

            void Register()
            {
                OnEffectHitTarget += SpellEffectFn(spell_gen_friendlystep_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_gen_friendlystep_SpellScript();
    }
};
...
class spell_teleport_group : public SpellScriptLoader
{
    public:
        spell_teleport_group() : SpellScriptLoader("spell_teleport_group") { }

        class spell_teleport_group_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_teleport_group_SpellScript);
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (!group || caster->GetGUID() != group->GetLeaderGUID()) // If caster is not group leader do not execute any code
                {
                    caster->ToPlayer()->GetSession()->SendNotification("You must be the leader of your group in order to summon your members.");
                    return;
                }
                ...
                {
                    Player* grpMember = itr->getSource();

                    if (!grpMember || grpMember->IsBeingTeleported())
                        continue;

                    if (!grpMember || !grpMember->IsInWorld() || !grpMember->GetSession())
                    {
                        if (!isInGame)
                        {
                            caster->ToPlayer()->GetSession()->SendNotification("One or more of your group members are not online. Here follows a list of name(s).");
                            isInGame = true;
                        }

                        // This text will appear for every single offline player
                        caster->ToPlayer()->GetSession()->SendNotification("Name of player not online: %s", grpMember->GetName());
                        continue;
                    }

                    if (grpMember->InBattleground() || grpMember->InArena())
                    {
                        if (!isInBattleground)
                        {
                            caster->ToPlayer()->GetSession()->SendNotification("One or more of your group members are in a battleground or arena. Here follows a list of name(s).");
                            isInBattleground = true;
                        }

                        caster->ToPlayer()->GetSession()->SendNotification("Name of player in battleground/arena: %s", grpMember->GetName());
                        continue;
                    }

                    // We only send an accept request to the group members as the group leader is already at the spot (Mr. Obvious)
                    if (grpMember->GetGUID() != caster->GetGUID())
                    {
			   	float map = caster->GetMapId();
			   	float x = caster->GetPositionX();
			   	float y = caster->GetPositionY();
			   	float z = caster->GetPositionZ();

                        grpMember->SetSummonPoint(map, x, y, z);

                        WorldPacket data(SMSG_SUMMON_REQUEST, 8 + 4 + 4);
                        data << uint64(caster->GetGUID());
                        data << uint32(caster->GetZoneId());
                        data << uint32(300000); // Auto decline after 5 minutes
                        grpMember->GetSession()->SendPacket(&data); // Send data to member
                    }
                }
            }
            ...
            void Register()
            {
                OnEffectHit += SpellEffectFn(spell_teleport_group_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_teleport_group_SpellScript();
        }
};

class spell_gen_plr_level_up_one : public SpellScriptLoader
{
    public:
        spell_gen_plr_level_up_one() : SpellScriptLoader("spell_gen_plr_level_up_one") {}

        class spell_gen_plr_level_up_one_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_gen_plr_level_up_one_SpellScript);
			 ...
			 void HandleDummy(SpellEffIndex /*effIndex*/)
			 {
				...
				{
					 player->GetSession()->SendNotification("You are already level 255.");
					 player->DestroyItemCount(44728, 1, true);
					 return;
				}
				 
				uint8 newLevel = player->getLevel() + 1;
            ...
            void Register()
            {
                OnEffectHit += SpellEffectFn(spell_gen_plr_level_up_one_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_gen_plr_level_up_one_SpellScript();
        }
};
...
void AddSC_generic_spell_scripts()
{
    ...
    new spell_teleport_group();
    new spell_gen_plr_level_up_one();
    new spell_gen_friendlystep();
