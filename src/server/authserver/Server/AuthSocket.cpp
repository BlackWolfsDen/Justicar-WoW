...
{
    sLog->outBasic("AuthSocket::OnClose");
}
...
void AuthSocket::OnRead()
{
    ...
    while (1)
    {
        ...
        for (i = 0; i < AUTH_TOTAL_COMMANDS; ++i)
        {
            ...
            {
                sLog->outBasic("Got data for cmd %u recv length %u", (uint32)_cmd, (uint32)socket().recv_len());

                if (!(*this.*table[i].handler)())
                {
                    sLog->outBasic("Command handler failed for cmd %u recv length %u", (uint32)_cmd, (uint32)socket().recv_len());
                    return;
                }
                break;
            }
        ...
        {
            sLog->outError("Got unknown packet from '%s'", socket().getRemoteAddress().c_str());
            socket().shutdown();
            return;
        }
    }
...
{
    sLog->outBasic("Entering _HandleLogonChallenge");
    ...
    sLog->outBasic("[AuthChallenge] got header, body is %#04x bytes", remaining);
    ...
    sLog->outBasic("[AuthChallenge] got full packet, %#04x bytes", ch->size);
    sLog->outBasic("[AuthChallenge] name(%d): '%s'", ch->I_len, ch->I);
    ...
    else
    {
        ...
        if (res2)
        {
            ...
            {
                sLog->outBasic("[AuthChallenge] Account '%s' is locked to IP - '%s'", _login.c_str(), fields[3].GetCString());
                sLog->outBasic("[AuthChallenge] Player address is '%s'", ip_address.c_str());

                if (strcmp(fields[3].GetCString(), ip_address.c_str()))
                {
                    sLog->outBasic("[AuthChallenge] Account IP differs");
                    pkt << (uint8) WOW_FAIL_SUSPENDED;
                    locked = true;
                }
                else
                    sLog->outBasic("[AuthChallenge] Account IP matches");
            }
            else
                sLog->outBasic("[AuthChallenge] Account '%s' is not locked to ip", _login.c_str());
...
{
    sLog->outBasic("Entering _HandleLogonProof");
    // Read the packet
...
{
    sLog->outBasic("Entering _HandleReconnectChallenge");
    if (socket().recv_len() < sizeof(sAuthLogonChallenge_C))
        return false;
    ...
    uint16 remaining = ((sAuthLogonChallenge_C *)&buf[0])->size;
    sLog->outBasic("[ReconnectChallenge] got header, body is %#04x bytes", remaining);
    ...
    // Read the remaining of the packet
    socket().recv((char *)&buf[4], remaining);
    sLog->outBasic("[ReconnectChallenge] got full packet, %#04x bytes", ch->size);
    sLog->outBasic("[ReconnectChallenge] name(%d): '%s'", ch->I_len, ch->I);
...
{
    sLog->outBasic("Entering _HandleReconnectProof");
    // Read the packet
...
{
    sLog->outBasic("Entering _HandleRealmList");
...
{
    sLog->outBasic("Entering _HandleXferResume");
...
{
    sLog->outBasic("Entering _HandleXferCancel");
...
{
    sLog->outBasic("Entering _HandleXferAccept");
